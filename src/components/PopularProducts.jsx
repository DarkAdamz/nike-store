...
import ProductCards from '../UI assets/ProductCards'
    ...
    <section id='products' className='max-container max-sm:mt-12'>
      ...
      <div className='mt-16 grid lg:grid-cols-4 md:grid-cols-3 sm:grid-cols-2 grid-cols-1 gap-14'>
        {products.map((product) => (
          <ProductCards key={product.name} {...product} imgUrl={product.imgURL} name={product.name} price={product.price} rating={product.rating}/>
        ))}
      </div>
    </section>
  )
